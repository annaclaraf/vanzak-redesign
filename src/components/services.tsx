        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((service, index) => (
            <RevealText key={index} delay={index * 0.1}>
              <div className="relative h-full bg-[#1c1e22] border border-[#f8fafc]/10 rounded-lg p-6 overflow-hidden">
                <div className="text-[#0638e5] mb-3 md:mb-6">{service.icon}</div>
                <h3 className="text-lg md:text-xl font-bold mb-3">{service.title}</h3>
                <p className="text-[#f8fafc]/70 text-sm md:text-[16px] mb-6">{service.description}</p>

                <MagneticLink href={`service/${service.slug}`} className="text-[#f8fafc] font-medium hover:text-[#0638e5] transition-colors flex items-center gap-2 w-fit">
